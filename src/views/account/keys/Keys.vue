...
export default {
  ...
  methods: {
    ...
    removeItem(index) {
      ...
      this.$http.get(`${this.$serverApiLink}api/account/signs/remove_ios/${this.list[index].uid}`).
      then(
    ...
    saveItem() {
      ...
      this.$http.post(
          `${this.$serverApiLink}api/account/signs/upload_cert`, params, {headers: {'Content-Type': 'multipart/form-data' }}).
      then(
          response => {
    ...
    getSigns() {
      this.loading = true;
      this.$http.get(`${this.$serverApiLink}api/account/signs/list_ios`).
      then(
          response => {
